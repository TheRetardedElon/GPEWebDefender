...
func (s *Store) initSearch() error {
	_, err := s.db.Exec(`CREATE VIRTUAL TABLE IF NOT EXISTS search_idx USING fts5(
		bucket, ref, num, src_ip, user, host, source, path, url, title, evidence, kind, category
	)`)
	if err != nil {
		return err
	}
	...
	_, err = s.db.Exec(`INSERT INTO search_idx(bucket, ref, num, src_ip, user, host, source, path, url, title, evidence, kind, category)
		SELECT 'event', id, '', IFNULL(src_ip,''), IFNULL(user,''), IFNULL(host,''), IFNULL(source,''),
		       IFNULL(path,''), IFNULL(url,''), '', '', IFNULL(kind,''), ''
		FROM events`)
	if err != nil {
		return err
	}
	_, err = s.db.Exec(`INSERT INTO search_idx(bucket, ref, num, src_ip, user, host, source, path, url, title, evidence, kind, category)
		SELECT 'alert', id, CAST(IFNULL(num,0) AS TEXT), IFNULL(src_ip,''), '', '', IFNULL(source,''),
		       '', IFNULL(url,''), IFNULL(title,''), IFNULL(evidence,''), '', IFNULL(category,'')
		FROM alerts`)
	return err
}
...
func (s *Store) indexEvent(ev event.Event) {
	...
	_, _ = s.db.Exec(`INSERT INTO search_idx(bucket, ref, num, src_ip, user, host, source, path, url, title, evidence, kind, category)
		VALUES('event', ?, '', ?, ?, ?, ?, ?, ?, '', '', ?, '')`,
		ev.ID, ev.SrcIP, ev.User, ev.Host, ev.Source, ev.Path, ev.URL, ev.Kind)
}
...
func (s *Store) indexAlert(al event.Alert) {
	...
	_, _ = s.db.Exec(`INSERT INTO search_idx(bucket, ref, num, src_ip, user, host, source, path, url, title, evidence, kind, category)
		VALUES('alert', ?, ?, ?, '', '', ?, '', ?, ?, ?, '', ?)`,
		al.ID, fmt.Sprintf("%d", al.Num), al.SrcIP, al.Source, al.URL, al.Title, al.Evidence, al.Category)
}
...
func (s *Store) Search(q SearchQuery) (event.SearchPage, error) {
	...
	sql := `SELECT bucket, ref, num, src_ip, user, host, source, path, url, title, kind, category
		FROM search_idx WHERE ` + strings.Join(cond, " AND ") + ` LIMIT ?`
	...
	for rows.Next() {
		...
		var num string
		if err := rows.Scan(&h.Bucket, &h.Ref, &num, &h.SrcIP, &h.User, &h.Host, &h.Source, &h.Path, &h.URL, &h.Title, &h.Kind, &h.Category); err != nil {
			return event.SearchPage{}, err
		}
